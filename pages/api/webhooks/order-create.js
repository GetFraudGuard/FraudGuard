...
async function makeApiRequest(endpoint, data, ignoreErrors = false) {
  try {
    const response = await axios.post(`${process.env.HOST}/api/${endpoint}`, data, {
    ...
    if (ignoreErrors) {
      console.warn(`Ignored non-critical /api/${endpoint} fetch error:`, errorMessage, { category: 'webhook-order-create' });
      return { success: false, error: errorMessage };
    }
    console.error(`Error in makeApiRequest for /api/${endpoint}:`, errorMessage, { category: 'webhook-order-create' });
    throw error;
  }
}
...
async function handleFlaggedOrder(db, orderData, shop, riskLevel, riskSettings, shopifyRiskAssessments, orderTxnDetails) {
  ...
  if (existingOrder) {
    ...
    const bin = await getBinFromOrderId(shop, orderData.id, session);
    const cardBrand = await getCardBrandFromBin(bin.bin);
...
async function fetchRiskSettings(shop) {
  try {
    const response = await axios.get(`${process.env.HOST}/api/settings/risk-settings?shop=${shop}`);
    return response.data;
  } catch (error) {
    const errorText = error.response?.data || error.message;
    console.error(`Failed to fetch risk settings for ${shop}: ${errorText}`, { category: 'webhook-order-create' });
    return {};
  }
}
