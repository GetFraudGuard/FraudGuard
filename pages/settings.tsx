    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50 flex">
        ...
        <main className="flex flex-col w-full items-center py-8 px-20 space-y-8">
          ...
          <div className="relative flex flex-col w-full max-w-6xl mx-auto mb-4 bg-white rounded-lg p-6 shadow-lg">
            <div className='flex items-center'>
              <LucideCircleCheckBig className='text-green-600 w-5 h-5' />
              <span className='ml-3 text-lg font-semibold'>Automated Actions</span>
            </div>

            {/* Auto-Cancel Unverified Orders Section */}
            <div className='flex flex-col mt-3'>
              <div className='flex items-center justify-between mt-4'>
                <div className='flex flex-col'>
                  ...
                  <span className='text-sm text-slate-600'>
                    When the verification link to provide correct details, the order will be auto-cancelled
                  </span>
                </div>
                <div className="flex items-center">
                  <button
                    type="button"
                    className={`relative inline-flex px-[2px] h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${autoCancelUnverified ? 'bg-black' : 'bg-gray-200'} ${autoCancelUnverifiedLoading ? 'opacity-60 pointer-events-none grayscale' : ''}`}
                    onClick={() => handleAutoActionChange('autoCancelUnverified', !autoCancelUnverified)}
                    disabled={autoCancelUnverifiedLoading}
                  >
                    <span
                      className={`inline-block h-5 w-5 transform rounded-full shadow-md bg-white transition-transform ${autoCancelUnverified ? 'translate-x-5' : ''}`}
                    />
                  </button>
                </div>
              </div>
              ...
              <div
                className={`transition-all duration-300 ease-in-out ${autoCancelUnverified
                  ? 'opacity-100 max-h-20 translate-y-0'
                  : 'opacity-0 max-h-0 translate-y-2 overflow-hidden'
                  }`}
              >
                <div className='flex bg-yellow-50 border border-yellow-200 p-4 rounded-md shadow-sm mt-4'>
                  <div className='flex items-center'>
                    <LucideTriangleAlert className='text-black w-4 h-4' />
                    <span className='ml-3 text-sm text-yellow-700'>
                      When this is enabled and the customer fails to provide correct details, the order will be auto-cancelled
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* Auto approve verified orders Section */}
            <div className='flex flex-col mt-3'>
              <div className='flex items-center justify-between mt-4'>
                <div className='flex flex-col'>
                  <span className='text-md text-slate-800 font-semibold'>
                  ...
                  <span className='text-sm text-slate-600'>
                    Automatically approve orders when customer verification is successful
                  </span>
                </div>
                <div className="flex items-center">
                  <button
                    type="button"
                    className={`relative inline-flex px-[2px] h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${autoApproveVerified ? 'bg-black' : 'bg-gray-200'} ${autoApproveVerifiedLoading ? 'opacity-60 pointer-events-none grayscale' : ''}`}
                    onClick={() => handleAutoActionChange('autoApproveVerified', !autoApproveVerified)}
                    disabled={autoApproveVerifiedLoading}
                  >
                    <span
                      className={`inline-block h-5 w-5 transform rounded-full shadow-md bg-white transition-transform ${autoApproveVerified ? 'translate-x-5' : ''}`}
                    />
                  </button>
                </div>
              </div>
            ...
            {/* Auto cancel High Risk Orders Section */}
            <div className='flex flex-col mt-3'>
              <div className='flex items-center justify-between mt-4'>
                <div className='flex flex-col'>
                  <span className='text-md text-slate-800 font-semibold'>
                  ...
                  <span className='text-sm text-slate-600'>
                    High risk order score 70+ from FraudGuard
                  </span>
                </div>
                <div className="flex items-center">
                  <button
                    type="button"
                    className={`relative inline-flex px-[2px] h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${autoCancelHighRisk ? 'bg-black' : 'bg-gray-200'} ${autoCancelHighRiskLoading ? 'opacity-60 pointer-events-none grayscale' : ''}`}
                    onClick={() => handleAutoActionChange('autoCancelHighRisk', !autoCancelHighRisk)}
                    disabled={autoCancelHighRiskLoading}
                  >
                    <span
                      className={`inline-block h-5 w-5 transform rounded-full shadow-md bg-white transition-transform ${autoCancelHighRisk ? 'translate-x-5' : ''}`}
                    />
                  </button>
                </div>
              </div>
              ...
              <div
                className={`transition-all duration-300 ease-in-out ${autoCancelHighRisk
                  ? 'opacity-100 max-h-20 translate-y-0'
                  : 'opacity-0 max-h-0 translate-y-2 overflow-hidden'
                  }`}
              >
                <div className='flex bg-red-50 border border-red-200 p-4 rounded-md shadow-sm mt-4'>
                  <div className='flex items-center'>
                    <LucideTriangleAlert className='text-black w-4 h-4' />
                    <span className='ml-3 text-sm text-red-700 font-bold'>
                      Warning: <span className='font-normal'>High risk orders will be automatically cancelled without customer verification opportunity</span>
                    </span>
                  </div>
                </div>
              </div>
